...
var gfProxyInit = (function (leadersocket) {
    ...
    //info message has been sent from server. for now just log it.
    ...
    function proxyFunction(fn, fnname, args, rec, params) {
        ...
        for (var i = 0, l = args.length; i < l; i++) {
            ...
            if (isEvent(args[i])) {
                ...
                var xy = normalizeMouseXY(basicEventOpts);
                //add to params
                fnargs['x'] = xy[0];
                fnargs['y'] = xy[1];
    ...
    function applyInterceptions(o) {
        ...
        var fnproperties = properties.filter(function (prop) {
            ...
            return typeof o[prop] === "function" && (prop.lastIndexOf("onmouse", 0) === 0);
        });
        ...
        for (var i = 0; i < o.attributes.length; i++) {
            var attrib = o.attributes[i];
            if (attrib.specified) {
                if (typeof o[attrib.name] === "function" && (attrib.name.lastIndexOf("onmouse", 0) === 0))
                    fnproperties.push(attrib.name);
            }
        }
